import { BlankNode, DataFactory, Dataset, DatasetCore, DatasetCoreFactory, DatasetFactory, DefaultGraph, Literal,
  NamedNode, Quad, BaseQuad, BaseTriple, Sink, Source, Store, Stream, Triple, Term, Variable, Quad_Subject,
  Quad_Predicate, Quad_Object, Quad_Graph } from "rdf-js";
...
function test_datafactory() {
    ...
    const triple1: Triple = dataFactory.triple(term, term, term);
    const triple2: Quad = triple1;
    const quad1: Quad = dataFactory.quad(term, term, term, term);
    interface QuadBnode extends BaseQuad<Literal, BlankNode, NamedNode, NamedNode> {}
    interface TripleBnode extends BaseTriple<Literal, BlankNode, NamedNode> {}
    const quad2: QuadBnode = dataFactory.quad<QuadBnode>(literal1, blankNode1, term, term);
    const hasBnode1 = quad2.predicate.termType === "BlankNode";
    const triple3: TripleBnode = dataFactory.triple<TripleBnode>(literal1, blankNode1, term);
    const hasBnode2 = triple3.predicate.termType === "BlankNode";
}
